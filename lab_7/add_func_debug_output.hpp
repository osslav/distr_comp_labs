...
void randomizeMatr(BoolMatr& matr, const uint32_t* size_matr)
{
    matr.clear();
    ...
    for (uint32_t i = 0; i < size_matr[0]; ++i)
    {
        ...
        for (uint32_t j = 0; j < size_matr[1]; ++j)
            matr[i][j] = static_cast<bool>(rand() % 2);
    }
}
...
void storeToBuf(uint8_t* buffer, uint32_t size_buf, const BoolArr& bool_arr)
{
    ...
    for (uint32_t ind_elem = 0; ind_elem < bool_arr.size(); ++ind_elem)
    {
        ...
        {
            std::cout << static_cast<uint32_t>(buffer[ind_curr_elem_buff]) << " ";
...
void main_runtime_func(uint32_t count_process)
{
    ...
    BoolMatr matrix;
    randomizeMatr(matrix, size_matr);
...
void worker_runtime_func(uint32_t ind_main)
{
    ...
    std::cout << "Size recieved " << size_matr[0] << " " << size_matr[1] << std::endl;
    ...
    std::cout << "Worker output:" << std::endl;
    printMatr(matrix);
